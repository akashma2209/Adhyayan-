...
//Join Class Button
app.post('/joinclass_st', encoder, (req, res) => {
...
app.get("/to_do_st", function (req, res) {
  res.render("todo_st");
})
...
// Assignment Page Button
app.get("/to_do_pr", function (req, res) {
  //res.render("todo");
  db.query("SELECT * FROM classassign_pr WHERE Inst_Id = ?", [userid_pr], function (err, results, fields) {
    if (err) {
      console.log('Error retrieving data from SQL table:', err);
      res.send('Error retrieving data from SQL table.');
    } else {
      var x = results.length;
      res.render('todo', { user: results, x: x });
    }
  });
})
...
app.post('/createassign', encoder, (req, res) => {
  var class_assign = req.body.class_assign;
  var description = req.body.Description;
  var uploadpr = "";

  //var uploadpr = req.files.uploadpr.data; // read the file data
  console.log('class assign: ' + class_assign);
  console.log('Class Description: ' + description);

  // Generate a random class code between 100000 and 900000
  const assigncode = Math.floor(Math.random() * 800) + 100000;
  console.log(userid_pr);
  const query = `INSERT INTO classassign_pr (class_assign ,description,  uploadpr, assigncode, Inst_id) VALUES (?, ?, ?, ?, ?)`;
  db.query(query, [class_assign, description, uploadpr, assigncode, userid_pr], (err, result) => {
    if (err) {
      console.error('Error inserting data into database:', err);
      res.sendStatus(500);
    } else {
      console.log('Data inserted into database');
      res.redirect("/to_do_pr");
